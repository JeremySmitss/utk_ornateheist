...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Start de bankoverval",
        silentstart = "[~r~E~w~] Start ~g~SILENT~w~ bank heist",
        usethermal = "[~r~E~w~] Gebruik thermiet",
        usehack = "[~r~E~w~] Hack het systeem",
        usesearch = "[~r~E~w~] Zoeken",
        lootcash= "[~r~E~w~] Pak het geld van de trolley",
        lootgold = "[~r~E~w~] Pak het goud van de trolley",
        lootdia = "[~r~E~w~] Pak het diamant van de trolley",
        thermal = "Thermiet aan het planten",
        burning = "Je bent het sleutelgat aan het smelten",
        using = "Systeem hack proces",
        used = "Hack is succesvol",
        stage = "Succesvol",
        search = "Zoeken",
        hacking = "Hacken",
        melted = "Sleutelgat is gesmolten",
        hacked = "Het systeem is succesvol gehacked",
        unlocked = "Deur is geopend",
        nothing = "Niks gevonden",
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                ESX.ShowNotification("Je hebt geen ~r~hackerstoestel")
                                            end
                                        end, "net_cracker")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            ESX.ShowNotification("Je hebt geen ~r~ID Kaart")
                                        end
                                        end, "id_card")
                                    end
                                end
                            ...
                            if not UTK.checks.thermal1 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                TriggerServerEvent("utk_oh:removeitem", "thermite")
                                                UTK.checks.thermal1 = true
                                                self.currentplant = 1
                                                self:Plant()
                                            elseif not output then
                                                ESX.ShowNotification("Je hebt geen ~r~thermiet")
                                            end
                                        end, "thermite")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    ESX.ShowNotification("Je hebt geen ~r~lockpick")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Plant()
    ...
    Citizen.Wait(9000)
    ESX.ShowNotification("~r~Je hebt het sleutelgat gesmolten", self.text.melted)
    print("melted keyprint")
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    ESX.ShowNotification("Open de computer en ga naar HackConnect.exe")
    print("hackconnent.exe runned")
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if IsScaleformMovieMethodReturnValueReady(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    ESX.ShowNotification("~b~Vind het IP adress...")
                    print("find IP adress")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    ESX.ShowNotification("~b~Vind het wachtwoord...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    ESX.ShowNotification("~g~Goedzo!~s~ Start BruteForce.exe op")
                    print("started BruteForce.exe")
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        ESX.ShowNotification("Het systeem is ~g~succesvol~s~ gehacked", UTK.text.hacked)
                    ...
                    elseif UTK.hackmethod == 2 then
                        ...
                        UsingComputer = false
                        Ipfinished = false
                        Process(25000, "Je bent de draden uit het systeem aan het halen...")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        ESX.ShowNotification("Je hebt ~y~250~s~ seconden voordat het gas naar binnen komt.")
        print("gas is coming")
        UTK.starttimer = true
        UTK.vaulttime = 250
        Citizen.CreateThread(function()
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            ESX.ShowNotification("Je hebt ~g~"..text.."~s~ gestolen")
        end)
    else
        ESX.ShowNotification("Je hebt ~y~250~s~ seconden voordat het gas naar binnen komt.")
        print("gas is coming")
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        ESX.ShowNotification("De kluisdeur gaat automatisch ~r~dicht~s~ in ~y~60~s~ seconden!")
        print("vault locked")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                ESX.ShowNotification("De kluisdeur gaat ~r~dicht!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            ESX.ShowNotification("Er is een grote bankoverval gaande!", 10000)
        ...
        for k, v in ipairs(PoliceDoors) do
            ...
            if dst <= 50 then
                if v.locked then
                    ...
                    doortext = "[~r~E~w~] Doe de deur open"
                    state = false
                elseif not v.locked then
                    FreezeEntityPosition(v.obj, false)
                    doortext = "[~r~E~w~] Doe de deur dicht"
                    state = true
                end
                if PlayerData.job.name == "police" then
                    if dst <= 1.5 then
