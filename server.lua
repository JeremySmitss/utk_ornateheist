...
local mincash = 2500 -- minimum amount of cash a pile holds
local maxcash = 5000 -- maximum amount of cash a pile can hold
...
local cooldown = 7200 -- amount of time to do the heist again in seconds (30min)
            ...
            if copcount >= mincops then
                if method == 1 then
                    ...
                    if item >= 1 then
                        ...
                        TriggerClientEvent('esx:showNotification', source, 'Je hebt geen ~r~thermiet')
                    end
                elseif method == 2 then
                    ...
                    if item >= 1 then
                        ...
                        TriggerClientEvent('esx:showNotification', source, 'Je hebt geen ~r~lockpick')
                    end
                end
            else
                TriggerClientEvent('esx:showNotification', source, "Er moeten minimaal ~b~"..mincops.." agenten~s~ in de stad zijn om de bank te kunnen overvallen")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." wachten voor een nieuwe overval te plegen.")
        end
    else
        TriggerClientEvent('esx:showNotification', source, 'De bank is ~y~recent~s~ overvallen')
    end
end)
